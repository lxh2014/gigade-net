...
//到Controller獲取數據
var WCT1Store = Ext.create('Ext.data.Store', {
    autoDestroy: true, //自動消除
    pageSize: pageSize,
    model: 'gigade.WCT1',
    proxy: {
        type: 'ajax',
        url: '/WebContentType/WebContentTypelist1',
        reader: {
            type: 'json',
            root: 'data',
...
WCT1Store.on('beforeload', function () {
    Ext.apply(WCT1Store.proxy.extraParams, {
...
Ext.onReady(function () {
    ...
    ToolAuthority();
    WCT1Store.load({ params: { start: 0, limit: pageSize} });
});

/********************************************新增*****************************************/
onAddClick = function () {
    editFunction(null, WCT1Store);
}

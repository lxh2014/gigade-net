...
Ext.onReady(function () {
                                    ...
                                    if (Ext.getCmp('epaper_id').getValue() == "") {
                                        Ext.Msg.confirm("提示信息", "保存成功,是否關閉?", function (btn) {
                                            if (btn == "yes") {
                                                ...
                                                TranToUrl('/EpaperContent/EpaperContentList')
                                                actab.close();
                                            }
                                            else {
    ...
    function TFunctionID(url, name) {
        Ext.Ajax.request({
            url: '/Edm/GetfunctionCodeID',
            method: 'post',
            async: false,
            params: {
                functionCode: url,
    ...
    function TranToUrl(url) {
        var urlTran = url;
        var last_Title = window.parent.Ext.getCmp('ContentPanel').activeTab.name
        TFunctionID(url, last_Title);
        var panel = window.parent.parent.Ext.getCmp('ContentPanel');
        var copy = panel.down('#list');
        if (copy) {
            copy.close();
        }
        copy = panel.add({

...
namespace Admin.gigade.Controllers
{
    public class ScheduleServiceController : Controller
    {
            ...
            try
            {
                ...
                foreach (ScheduleMasterQuery item in store)
                {
                    try
                    {
                            ...
                            //執行排程
                            bool result = _secheduleServiceMgr.ExeScheduleService(item.api, store_config);
                            if (result)
                            {
                                //記錄排程執行記錄
        ...
        public HttpResponseBase SendEmailService()
        {

            MailModel mailModel = new MailModel();
            mailModel.MysqlConnectionString = ;
            if (!string.IsNullOrEmpty(Request.Params["MysqlConnectionString"]))
            {
                mailModel.MysqlConnectionString = Request.Params["MysqlConnectionString"].ToString();
            }
            ...
            {
                if(Request.Params["IsSeparate"].ToString()=="false")
                {
                    IsSeparate = false;
                }
                if(Request.Params["IsSeparate"].ToString()=="true")
                {
                    IsSeparate = true;
                }
            }
            ...
            {
                IsDisplyName = Request.Params["IsDisplyName"].ToString();
            }
            mail.SendToGroup(GroupCode, EmailTile, sbstr, false, true);

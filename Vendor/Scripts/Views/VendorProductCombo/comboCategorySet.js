...
function save(functionid) {
    ...
    Ext.Ajax.request({
        url: '/VendorProductCombo/tempCategoryAdd',
        method: 'POST',
        async: false,
        params: {
...
function createPanel() {
    ...
    var treeStore = Ext.create('Ext.data.TreeStore', {
        proxy: {
            type: 'ajax',
            url: '/VendorProductCombo/GetCatagory',
            actionMethods: 'post'
        },
        root: {
            expanded: true,
    ...
    var categoryPanel = Ext.create('Ext.form.Panel', {
        ...
        listeners: {
            beforerender: function () {
                Ext.Ajax.request({
                    url: '/VendorProductCombo/GetSelectedCage',
                    method: 'POST',
                    params: {
                        ProductId: PRODUCT_ID,
                        OldProductId: OLD_PRODUCT_ID
    ...
    stageCategoryPanel = Ext.create('Ext.tree.Panel', {
        ...
        minWidth: 200,
        width: 500,
        height: IsEdit == 'true' ? 320 : document.documentElement.clientHeight - 105,
        lines: true,
        enableDrag: true,
        store: treeStore
    });

...
var priceListStore = Ext.create("Ext.data.Store", {
    fields: ["product_image", "product_id", "brand_name",
             "product_name", "combination", "product_status",
             "site_name", "user_level", "user_email",
             "price_status", "price", "event_price", "cost", "event_cost",
             "event_start", "event_end", "apply_time", "apply_user", "price_master_id", "CanDo", "site_id", "level", "user"],
    autoLoad: false,
    pageSize: pageSize,
    proxy: {
        type: 'ajax',
        url: '/VendorProductList/QueryPriceVerifyList',
        actionMethods: 'post',
        reader: {
            type: 'json',
            root: 'data',
...
function showResultText(btn, text) {
    if (btn == "ok") {
        ...
        Ext.Ajax.request({
            url: '/VendorProductList/PriceVerify',
            method: 'post',
            params: {
                "type": 2,
                "reason": text,
            ...
            listeners: {
                click: function () {
                    ...
                    Ext.Ajax.request({
                        ...
                        params: {
                            "type": 1,
                            "prods": Ext.htmlEncode(prods.substring(0, prods.length - 1)),
                            "prices": Ext.htmlEncode(prices.substring(0, prices.length - 1)),
                            "function": 'pass'
                        },
                        success: function (response, opts) {

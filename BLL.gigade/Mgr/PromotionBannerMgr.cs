    ...
    public class PromotionBannerMgr
    {
        ...
        public void AllowShowOrNot(PromotionBannerQuery query, string[] bids, out int brand_id)
        {
            ...
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    List<PromotionBannerRelationQuery> pbr_store = _promotionBannerRelationDao.GetBrandIds(pb_ids);
                    for (int i = 0; i < pbr_store.Count; i++)
                    {
                        if (query.singleBrand_id == pbr_store[i].brand_id)
                        {
        ...
        public int AllowMultiOrNot(PromotionBannerQuery query,out int id)
        {
            ...
            try
            {
                ...
                string pb_ids = string.Empty;
                if (!string.IsNullOrEmpty(Using_Image.Rows[0]["pb_startdate"].ToString()))
                {
                    query.date_start = Convert.ToDateTime(Using_Image.Rows[0]["pb_startdate"].ToString());
                }
                if (!string.IsNullOrEmpty(Using_Image.Rows[0]["pb_enddate"].ToString()))
                {
                    query.date_end = Convert.ToDateTime(Using_Image.Rows[0]["pb_enddate"].ToString());
                }
                for (int a = 1; a < Using_Image.Rows.Count; a++)
                {
                    DateTime pb_startdate = Convert.ToDateTime(Using_Image.Rows[a]["pb_startdate"].ToString());
                    DateTime pb_enddate = Convert.ToDateTime(Using_Image.Rows[a]["pb_enddate"].ToString());
                        ...
                        pb_startdate >= query.date_start && pb_startdate <= query.date_end && pb_enddate >= query.date_end)
                    {
                        if (a == 1)
                        {
                            pb_ids = Using_Image.Rows[0]["pb_id"].ToString();
                        }
                        pb_ids += "," + Convert.ToInt32(Using_Image.Rows[a]["pb_id"].ToString());
                    }
                }              
                List<PromotionBannerRelationQuery> store = _promotionBannerRelationDao.GetBrandIds(pb_ids);
                for (int i = 0; i < store.Count; i++)
                {
                    for (int j = store.Count - 1; j > i; j--)
                    {
                        if (store[i].brand_id == store[j].brand_id)
                        {
                            id = store[i].brand_id;

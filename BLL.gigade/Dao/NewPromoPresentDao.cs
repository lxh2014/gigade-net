    ...
    public class NewPromoPresentDao : INewPromoPresentImplDao
    {
        ...
        public DataTable GetNewPromoPresentList(NewPromoPresentQuery query, out int totalCount)
        {
            ...
            {//from new_promo_present npp LEFT JOIN product pt on pt.product_id=npp.gift_id  LEFT JOIN vip_user_group vug on npp.group_id=vug.group_id 
                sql.Append(@" select temp.parameterName as event_type, npp.row_id as trow_id,npp.event_id,npp.welfare_mulriple,npp.gift_type,npp.ticket_name,npp.ticket_serial,npp.gift_id,npp.deduct_welfare,npp.gift_amount ,npp.gift_amount_over,npp.`status`,npp.freight_price,pt.product_name,npp.start as tstart,npp.end as tend,npp.group_id as tgroup_id ,npp.bonus_expire_day,npp.use_span_day,vug.group_name,npp.muser,mu.user_username ");
                sqlCondition.Append(" from new_promo_present npp ");
                sqlCondition.Append(" LEFT JOIN (SELECT parameterCode, parameterName FROM t_parametersrc  WHERE parameterType='new_promo_present_event_type') AS temp on temp.parameterCode=npp.event_type ");
        ...
        public string InsertNewPromoPresent(NewPromoPresentQuery store)
        {
            ...
            try
            {
                sql.Append(@"insert into new_promo_present (event_id,gift_type,group_id,start,end,ticket_name,ticket_serial,gift_id,deduct_welfare,row_id,gift_amount ,gift_amount_over,status,freight_price,bonus_expire_day,welfare_mulriple,created,modified,kuser,muser,use_span_day,event_type)");
                ...
                sql.AppendFormat(@",'{0}','{1}',{2},{3},{4},'{5}');",CommonFunction.DateTimeToString( store.created),CommonFunction.DateTimeToString(store.modified),store.kuser,store.muser,store.use_span_day,store.event_type);
                return sql.ToString();
            }
            catch (Exception ex)
            {
        ...
        public string UpdateNewPromoPresent(NewPromoPresentQuery store)
        {
            ...
            try
            {
                ...
                sql.AppendFormat(@",ticket_name='{0}',ticket_serial='{1}',gift_id='{2}',deduct_welfare='{3}',welfare_mulriple='{4}',event_type='{5}'", store.ticket_name, store.ticket_serial, store.gift_id, store.deduct_welfare, store.welfare_mulriple,store.event_type);

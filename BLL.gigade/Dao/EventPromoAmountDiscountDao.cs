...
{
    class EventPromoAmountDiscountDao:IEventPromoAmountDiscountImplDao
    {
        private IDBAccess access;
        public EventPromoAmountDiscountDao(string connectionStr)
        {
        ...
        public List<Model.Query.EventPromoAmountDiscountQuery> GetList(Model.Query.EventPromoAmountDiscountQuery epQuery, out int totalCount)
        {
            ...
            try
            {
                ...
                sqlWhere.Append(" from event_promo_amount_discount eg ");
                ...
                if (epQuery.IsPage)
                {
                    DataTable _dt = access.getDataTable(" select count(eg.event_id) as totalCount " + sqlWhere.ToString());
                    if (_dt.Rows.Count > 0)
                    {
                ...
                return access.getDataTableForObj<Model.Query.EventPromoAmountDiscountQuery>(sql.ToString());
            }
            catch (Exception ex)
            {
                throw new Exception("EventPromoAmountDiscountDao-->GetList-->" + ex.Message, ex);
            }
        }
        public string AddOrUpdate(Model.EventPromoAmountDiscount model)
        {
            model.Replace4MySQL();
            StringBuilder sql = new StringBuilder();
            try
            {
                if (model.row_id == 0)
                {
                    sql.Append(@"insert into event_promo_amount_discount(event_name,event_desc,event_start,event_end,event_type,event_id,
                ...
                {
                    sql.AppendFormat("update event_promo_amount_discount set  event_name='{0}',event_desc='{1}',event_start='{2}',event_end='{3}',event_type='{4}',", model.event_name, model.event_desc, Common.CommonFunction.DateTimeToString(model.event_start), Common.CommonFunction.DateTimeToString(model.event_end), model.event_type);
            ...
            {
                throw new Exception("EventPromoAmountDiscountDao-->AddOrUpdate-->" + ex.Message, ex);
            }

        }
        public string UpdateActive(Model.EventPromoAmountDiscount model)
        {
            StringBuilder sql = new StringBuilder();
            try
            {
                model.Replace4MySQL();
                sql.AppendFormat("set sql_safe_updates = 0;update event_promo_amount_discount set event_status ='{0}', modify_user='{1}',modify_time='{2}'  where event_id = '{3}';set sql_safe_updates = 1;", model.event_status, model.modify_user, Common.CommonFunction.DateTimeToString(model.modify_time), model.event_id);

                return sql.ToString();
            }
            catch (Exception ex)
            {
                throw new Exception("EventPromoAmountDiscountDao-->UpdateActive-->" + ex.Message + sql.ToString(), ex);
            }
        }
        ...
        public string UpdateEventId(int row_id, string event_id)
        {
            ...
            try
            {
                sql.AppendFormat("set sql_safe_updates = 0;update event_promo_amount_discount set event_id ='{0}' where row_id = '{1}';set sql_safe_updates = 1;", event_id, row_id);
                return sql.ToString();
            }
            catch (Exception ex)
            {
                throw new Exception("EventPromoAmountDiscountDao-->UpdateEventId-->" + ex.Message + sql.ToString(), ex);
            }
        }

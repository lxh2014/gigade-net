                ...
                foreach (KeyValuePair<string, string> item in iinvd)
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        {//刪除收貨上架表庫存,往iwms_record表添加數據
                            ...
                            sb.AppendFormat("insert into istock_change(sc_trans_id,item_id,sc_trans_type,sc_num_old,sc_num_chg,sc_num_new,sc_time,sc_user) Values ('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}');", a.ord_id, dt.Rows[0]["item_id"], "3", sum, "-" + value.ToString(), sum - value, CommonFunction.DateTimeToString(DateTime.Now), a.change_user);
                            sum = sum - value;
                        }

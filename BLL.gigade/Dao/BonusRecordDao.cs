...
namespace BLL.gigade.Dao
{
    public class BonusRecordDao : IBonusRecordImplDao
    {
        ...
        public uint GetRecordID()
        {
            ...
            try
            {
                strSql.Append(@"SELECT MAX(record_id) FROM bonus_record;");
                return Convert.ToUInt32(_dbAccess.getDataTable(strSql.ToString()).Rows[0][0].ToString());
            }
            catch (Exception ex)
            {
                throw new Exception("BonusMasterDao-->GetRecordID()-->" + strSql.ToString() + ex.Message, ex);
            }
        }
        #endregion

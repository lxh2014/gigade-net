...
namespace BLL.gigade.Dao
{
    public class ProductConsultDao : IProductConsultImplDao
    {
        ...
        public int UpdateAnswerStatus(ProductConsultQuery model)
        {
            ...
            try
            {
                strSql = string.Format("set sql_safe_updates = 0; update product_consult set answer_status='{0}',answer_user='{1}',delay_reason='{2}',status={3} where consult_id='{4}'; set sql_safe_updates = 1;", model.answer_status, model.answer_user, model.delay_reason, model.status, model.consult_id);
                return _access.execCommand(strSql);
            }
            catch (Exception ex)
            {

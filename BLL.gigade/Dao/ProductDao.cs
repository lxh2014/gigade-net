...
//商品匯出添加商品上下架備註欄位 add by mingwei0727w 2015/09/25
                        ...
                        strTbls.AppendFormat(" LEFT JOIN (select product_id,remark from product_status_history where id in( select max(id) from product_status_history where type = 1 and remark <> '' group by product_id )) as psh on a.product_id = psh.product_id ");

...
namespace BLL.gigade.Dao
{
    public class IstockChangeDao
    {
        ...
        public List<IstockChangeQuery> GetIstockChangeList(IstockChangeQuery query, out int totalCount)
        {
            ...
            List<IstockChangeQuery> IstockChangeList=new  List<IstockChangeQuery>();
            ...
            sbAll.Append("  ,CONCAT(p.spec_title_1,'-',ps.spec_name)as specname,CONCAT(p.spec_title_2,'-',ps2.spec_name)as specname2 ");
            ...
            try
            {
                ...
                IstockChangeList = _accessMySql.getDataTableForObj<IstockChangeQuery>(sbAll.ToString() + sbJoin.ToString() + sbWhr.ToString() + sbPage.ToString());
                foreach (IstockChangeQuery item in IstockChangeList)
                {
                    item.specname = string.IsNullOrEmpty(item.specname) ? item.specname2 : item.specname + item.specname2;
                }
                return IstockChangeList;
            }
            catch (Exception ex)
            {

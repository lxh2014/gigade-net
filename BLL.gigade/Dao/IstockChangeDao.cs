...
namespace BLL.gigade.Dao
{
    public class IstockChangeDao
    {
        ...
        public List<IstockChangeQuery> GetIstockChangeList(IstockChangeQuery query, out int totalCount)
        {
            ...
            sbAll.Append("SELECT ic.item_id,ic.sc_trans_id,ic.sc_cd_id,(CASE WHEN ic.sc_trans_type=1 THEN '收貨上架' WHEN ic.sc_trans_type=2 THEN '庫存調整' WHEN ic.sc_trans_type=3 THEN 'RF理貨' when ic.sc_trans_type=4 then '出盤復盤'END) AS typename,(CASE WHEN ic.sc_istock_why=1 THEN '庫鎖' WHEN ic.sc_istock_why=2 THEN '庫調' else '' END) AS istockwhy,ic.sc_num_chg,ic.sc_num_new,ic.sc_time,ic.sc_note,p.product_name,mu.user_username AS manager ");

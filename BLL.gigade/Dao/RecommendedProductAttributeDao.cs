...
namespace BLL.gigade.Dao
{
    public class RecommendedProductAttributeDao
    {
        ...
        public string DeleteProductAttribute(int productId)
        {
            ...
            try
            {
                strSql.AppendFormat("SET sql_safe_updates = 0; DELETE FROM recommended_product_attribute WHERE product_id = '{0}';SET sql_safe_updates = 1;", productId);
                return strSql.ToString();
            }
            catch (Exception ex)
            {
                throw new Exception("RecommendedProductAttributeDao-->DeleteProductAttribute-->" + ex.Message + strSql.ToString(), ex);
            }
        }

...
Ext.define('Ext.calendar.form.EventWindow', {
    ...
    onSave: function(){
        ...
        Ext.Ajax.request({
            url: '/MattersCalendar/OperateEvent',
            params: {
                paraType: this.activeRecord.phantom ? 'eventadd' : 'eventupdate',
                startTime: (this.activeRecord.data.StartDate.getTime()/1000).toString(),
                endTime: (this.activeRecord.data.EndDate.getTime()/1000).toString(),
                id: this.activeRecord.data.id,
                title: this.activeRecord.data.Title,
                notes: this.activeRecord.data.Notes,
                calendarId: this.activeRecord.data.CalendarId
            },
    ...
    // private
    onDelete: function () {
        //Ext.Msg.confirm('消息提示', '您確認刪除該條記錄?', function (btns) {
        //    var btns = btns;
        //    if (btns == 'yes') {
        //        Ext.Ajax.request({
        //            url: '/MattersCalendar/OperateEvent',

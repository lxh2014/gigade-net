...
Ext.define('gigade.Fares', {
    ...
    fields: [
        ...
        { name: "PointDollars", type: "string" },
        { name: "newstart", type: "string" },
        { name: "payment_type_rid", type: "string" },
...
var FaresStore = Ext.create('Ext.data.Store', {
    ...
    proxy: {
        type: 'ajax',
        url: '/PromotionsAccumulateRate/PromotionsAccumulateRatelist',
        reader: {
            type: 'json',
            root: 'data',
...
Ext.onReady(function () {
    var gdFgroup = Ext.create('Ext.grid.Panel', {
        ...
        columns: [
            ...
            { header: DOLLARPOINT, dataIndex: 'PointDollars', width: 100, align: 'center' },
            { header: PAYTYPE, dataIndex: 'payment_name', width: 200, align: 'center' },

        //            { header: '支付方式', dataIndex: 'payment_type_rid', width: 100, align: 'center' },
            { header: BEGINTIME, dataIndex: 'newstart', width: 200, align: 'center' },
    ...
    if (row.length < 0) {
        ...
        Ext.Msg.confirm(CONFIRM, Ext.String.format(DELETE_INFO, row.length), function (btn) {
            if (btn == 'yes') {
                ...
                Ext.Ajax.request({
                    url: '/PromotionsAccumulateRate/Delete',
                    method: 'post',
                    params: { rowID: rowIDs },
...
function UpdateActive(id,muser) {
    ...
    $.ajax({
        url: "/PromotionsAccumulateRate/UpdateActive",
        data: {
            "id": id,
            "muser": muser,
            "active": activeValue

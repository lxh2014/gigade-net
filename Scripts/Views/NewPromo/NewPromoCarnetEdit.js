...
function editFunction(row, store) {
    ...
    var secondForm = Ext.widget('form', {
        ...
        defaultType: 'textfield',
        layout: 'anchor',
        url: '/NewPromo/InsertNewPromoCarnet',
        ...
        items: [
                             ...
                             if (newValue != "") {//驗證贈品編號是否合法
                                 Ext.Ajax.request({
                                     ...
                                     success: function (form, action) {
                                         ...
                                         if (result.success) {
                                             if (result.count == 0) {
                                                 Ext.Msg.alert(INFORMATION, GIFTSUMITTIP);
            ...
            },
            {
                ...
                items: [
                    {
                        boxLabel: MSG, id: 'message', inputValue: '0', checked: true
                    },
                   {
                       boxLabel: SECRET, id: 'secret', inputValue: '1'
                   }
                ]
            },
            {
        ...
        buttons: [{
            ...
            handler: function () {
                ...
                if (Ext.getCmp('present_event_id').getValue() != "") {
                    ...
                    if (boolReturn) {
                        Ext.Msg.confirm(CONFIRM, GIFTTIPTWO, function (btn) {
                            if (btn == 'no') {
                                return false;
                            } else {
                                if (form.isValid()) {
                                    ...
                                    form.submit({
                                        params: {
                                            ...
                                            message_mode: Ext.htmlEncode(Ext.getCmp('message_mode').getValue().m_message_mode),
    ...
    function initForm(Row) {
        ...
        if (Row.data.message_mode == 0) {
            Ext.getCmp('message').setValue(true);
        }
        else {
            Ext.getCmp('secret').setValue(true);
        }

        if (Row.data.active_now == '否') {
